...
/**
 * AutowireCapableBeanFactory：可自动的BeanFactory
 * bean factories that are capable of autowiring, provided that they want to expose this functionality for existing bean instances.
 *
...
public interface AutowireCapableBeanFactory extends BeanFactory {
	...
	/**
	 * 自动绑定模式常量值
	 * 1）no：IoC容器默认的自动绑定模式，不采用任何形式的自动绑定，完全依赖用户手工明确各个Bean之间的依赖关系
	 * 2）byName：按照用户声明的实例变量的名称、Bean定义的bean名称进行匹配，相匹配的Bean定义将会自动绑定到当前实例变量上，注意要保证只存在1个符合条件的
	 * 3）byType：容易根据当前Bean定义类型，分析其依赖的对象类型，然后到容器中寻找与依赖对象类型相同的Bean定义，然后自动绑定，注意要保证只存在1个符合条件的
	 * 4）constructor：与上面不同，四针对构造方法参数的类型而进行的自动绑定，同样是byType类型的绑定方式，注意要保证只存在1个符合条件的
	 * 5）autodetect：byType 和 constructor 模式的结合体
	 */
	int AUTOWIRE_NO = 0;
